...
function Shell({ children }: { children: ReactNode }) {
  ...
  const active = (href: string) => (href === '/workspace' ? location === '/workspace' || location === '/' : location.startsWith(href));
  return (
...
function WorkspaceHome() {
  ...
  return (
    <div className="mx-auto max-w-[1260px]">
      ...
      {dash.isError && <Notice tone="error">Could not load dashboard.</Notice>}
      {dash.data && (
...
function BrandDNA() {
  ...
  return (
    <div className="mx-auto max-w-[900px]">
      <PageHeading eyebrow="Brand DNA" title="Define the brand brain." body="Campaigns and creatives attach to this brand. Fill this in before creating a campaign." />
...
function Campaigns() {
  ...
  return (
    <div className="mx-auto max-w-[1260px]">
      ...
      {list.isError && <Notice tone="error">Could not load campaigns.</Notice>}
  ...
  return (
    <div className="mx-auto max-w-[900px]">
      ...
      <div className="space-y-4 rounded-2xl border border-[#2b2e22] bg-[#1a1c16] p-6">
        ...
        {mutation.isError && <Notice tone="error">{String((mutation.error as any)?.message || (mutation.error as any)?.data?.error || 'Could not create campaign. Create Brand DNA first and fill required fields.')}</Notice>}
      </div>
    </div>
  );
}
...
function CampaignDetail() {
  ...
  return (
    <div className="mx-auto max-w-[1100px]">
      <PageHeading eyebrow="Campaign" title={campaign?.name || 'Campaign'} body={campaign?.description || 'Loading…'} action={
        <div className="flex gap-2">
          <button className={buttonGhost} disabled={genConcepts.isPending} onClick={() => genConcepts.mutate({ campaignId: id })}>{genConcepts.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : null}Generate concepts</button>
          <button className={buttonPrimary} disabled={genCreatives.isPending} onClick={() => genCreatives.mutate({ data: { campaignId: id } as any })}>{genCreatives.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : null}Generate creatives</button>
        </div>
      } />
      {campaignQ.isError && <Notice tone="error">Campaign not found.</Notice>}
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        {((conceptsQ.data || []) as AnyRecord[]).map((c) => (
          <div key={c.id} className="rounded-2xl border border-[#2b2e22] bg-[#1a1c16] p-5"><div className="text-xs text-[#d7f36b]">{c.family}</div><h3 className="mt-2 font-semibold">{c.name}</h3><p className="mt-2 text-sm text-[#9b9d89]">{c.hook}</p></div>
        ))}
      </div>
    </div>
  );
...
function CreativeLab() {
  const list = useListCreatives();
  const creatives = (list.data || []) as AnyRecord[];
  return (
    <div className="mx-auto max-w-[1260px]">
      <PageHeading eyebrow="Creative lab" title="The good stuff, in one place." body="Creatives generated from your campaigns." action={<Link href="/campaigns/new" className={buttonPrimary}><Plus className="h-4 w-4" />New campaign</Link>} />
      {creatives.length === 0 && <EmptyState title="No creatives yet" body="Create a campaign and generate creatives." />}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{creatives.map((c) => (
        <div key={c.id} className="overflow-hidden rounded-2xl border border-[#2b2e22] bg-[#1a1c16]">
          ...
          <div className="p-4"><div className="text-xs text-[#d7f36b]">{c.family}</div><h3 className="mt-1 font-semibold">{c.headline}</h3><p className="mt-1 text-sm text-[#9b9d89]">{c.hook}</p></div>
        </div>
      ))}</div>
    </div>
  );
...
function SettingsPage() {
  return <div className="mx-auto max-w-[700px]"><PageHeading eyebrow="Settings" title="Workspace" body="Account and studio preferences." /><Notice>Studio is live. Brand, campaigns, and creatives are stored in your Neon database.</Notice></div>;
}
