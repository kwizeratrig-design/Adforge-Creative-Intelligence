...
import {
  ArrowUpRight, Loader2, LogOut, Plus,
} from 'lucide-react';
...
function CampaignAssetPicker({ selectedIds, onChange }: { selectedIds: string[]; onChange: (ids: string[]) => void }) {
  ...
  const upload = async (file: File) => {
    ...
    try {
      ...
      if (!previewUrl) throw new Error('Could not read image file');
      const asset = await createAsset.mutateAsync({
        data: {
          name: file.name,
          type: file.type.startsWith('image/') ? 'image' : (file.type || 'image'),
          objectPath: null,
          previewUrl,
      ...
      console.error(err);
      alert(String(err?.data?.error || err?.message || 'Could not upload media.'));
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-[900px]">
      ...
      <div className="space-y-4 rounded-2xl border border-[#2b2e22] bg-[#1a1c16] p-6">
        ...
        {(createBrand.isError || updateBrand.isError) && <Notice tone="error">{String((createBrand.error as any)?.data?.error || (updateBrand.error as any)?.data?.error || (createBrand.error as any)?.message || (updateBrand.error as any)?.message || 'Could not save brand.')}</Notice>}
