...
async function generateConcepts(userId: string, campaignId: string) {
  ...
  if (!campaign) return [];
  const existing = await db.select().from(creativeConceptsTable).where(eq(creativeConceptsTable.campaignId, campaignId));
...
router.get("/dashboard", async (req, res) => {
  ...
  const campaigns = await db.select().from(campaignsTable).where(brandId ? eq(campaignsTable.brandId, brandId) : undefined).orderBy(desc(campaignsTable.createdAt)).limit(4);
  const creatives = await db.select().from(creativesTable).where(brandId ? inArray(creativesTable.campaignId, db.select({ id: campaignsTable.id }).from(campaignsTable).where(eq(campaignsTable.brandId, brandId))) : undefined).orderBy(desc(creativesTable.createdAt)).limit(8);
  const [campaignCount] = await db.select({ total: count() }).from(campaignsTable).where(brandId ? eq(campaignsTable.brandId, brandId) : undefined);
  const [creativeCount] = await db.select({ total: count() }).from(creativesTable).where(brandId ? inArray(creativesTable.campaignId, db.select({ id: campaignsTable.id }).from(campaignsTable).where(eq(campaignsTable.brandId, brandId))) : undefined);
  const [savedCount] = await db.select({ total: count() }).from(creativesTable).where(and(eq(creativesTable.isFavorite, true), brandId ? inArray(creativesTable.campaignId, db.select({ id: campaignsTable.id }).from(campaignsTable).where(eq(campaignsTable.brandId, brandId))) : undefined));
  res.json(GetDashboardResponse.parse({
    ...
    recentCreatives: creatives.map(serializeCreative),
    activity: brand ? [{ id: "activity-1", label: "Brand ready", detail: `${brand.name} is ready for new work.`, timestamp: "Today", type: "brand" }] : [{ id: "activity-1", label: "Create your brand", detail: "Set up Brand DNA to start generating creatives.", timestamp: "Now", type: "brand" }],
  }));
});
...
router.post("/brands/current", async (req, res) => {
  ...
  if (!parsed.success) { res.status(400).json({ error: "Invalid brand details.", details: parsed.error.flatten() }); return; }
  const timestamp = now();
...
router.patch("/brands/current", async (req, res) => {
  ...
  if (!current) { res.status(404).json({ error: "Brand not found. Create your brand first." }); return; }
  const parsed = CreateBrandBody.safeParse(req.body);
  if (!parsed.success) { res.status(400).json({ error: "Invalid brand details.", details: parsed.error.flatten() }); return; }
...
router.delete("/brands/assets/:assetId", async (req, res) => {
  const brand = await getUserBrand(getAuth(req).userId!);
  if (!brand) { res.status(403).json({ error: "Create a brand first." }); return; }
...
router.get("/creatives", async (req, res) => {
  ...
  const creatives = brand ? await db.select().from(creativesTable).where(campaignId ? eq(creativesTable.campaignId, campaignId) : inArray(creativesTable.campaignId, db.select({ id: campaignsTable.id }).from(campaignsTable).where(eq(campaignsTable.brandId, brand.id)))).orderBy(desc(creativesTable.createdAt)) : [];
  res.json(ListCreativesResponse.parse(creatives.map(serializeCreative)));
});
...
router.post("/creatives/:creativeId/variations", async (req, res) => {
  const creative = await getUserCreative(getAuth(req).userId!, req.params.creativeId);
  if (!creative) { res.status(404).json({ error: "Creative not found." }); return; }
  const options: Array<[string, string, string, string]> = [["Hook", creative.hook, creative.headline, creative.cta], ["Headline", creative.headline, creative.headline, creative.cta], ["CTA", creative.hook, creative.headline, creative.cta || "Learn more"]];
  const variations = options.map(([dimension, label, headline, cta], index) => ({ id: randomUUID(), creativeId: creative.id, dimension, label, headline, bodyCopy: creative.bodyCopy, cta, previewUrl: placeholderImages[(index + 2) % placeholderImages.length], readinessScore: 90 + index, createdAt: now(), updatedAt: now() }));
...
router.get("/creative-intelligence", async (_req, res) => {
  res.json(GetCreativeIntelligenceResponse.parse({ isSampleData: true, totalImpressions: 0, averageCtr: 0, conversionRate: 0, winningFamily: "—", winningHook: "Generate creatives to unlock performance insights.", familyPerformance: [], weeklyPerformance: [], recommendations: ["Create a brand and campaign to start generating ads.", "Generate creatives to populate performance signals.", "Use variations to test hooks, headlines, and CTAs."] }));
});
